...
class SubscriberAppln:
    ...
    def configure(self, args):
        try:
            ...
            self.name = args.name
            self.args = args  # Store args for reconnection
            ts = TopicSelector()
        ...
        try:
            ...
            while True:  # Outer loop for reconnection
                try:
                    ...
                    # If we get here, either we completed normally or need to reconnect
                    if self.state == self.State.COMPLETED:
                        break  # Normal completion, exit the loop
                    
                    # Otherwise, we need to reconnect
                    self.logger.info("SubscriberAppln::driver - Reconnecting to load balancer")
                    self.state = self.State.REGISTER  # Reset state for reconnection
                    self.mw_obj.handle_events = True  # Reset event loop flag
                    
                    # Clean up existing connections
                    self.mw_obj.cleanup()
                    
                    # Reconfigure middleware
                    self.mw_obj.configure(self.args)
                    
                except Exception as e:
                    self.logger.error(f"Exception in driver: {str(e)}")
                    # If we get an exception, try to reconnect
                    self.state = self.State.REGISTER
                    self.mw_obj.handle_events = True
                    self.mw_obj.cleanup()
                    self.mw_obj.configure(self.args)
...
def parseCmdLineArgs():
    ...
    parser.add_argument("-m", "--max_latency", type=int, default=6, help="Maximum allowed latency in milliseconds before reconnecting")
